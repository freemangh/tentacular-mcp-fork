...
1. **Vault integration** -- SVIDs become auth credentials for HashiCorp Vault. Vault issues short-lived Postgres credentials, NATS tokens, and S3 presigned URLs. Replaces static passwords with time-limited secrets.
2. **Per-tentacle NATS JWT accounts** -- Full NATS account isolation with enforced publish/subscribe permissions per tentacle.
3. **Istio ambient mode** -- SPIFFE IDs become the service mesh identity layer. Zero-config mTLS for all tentacle-to-service traffic.
4. **Provenance persistence** -- Currently annotations only. Future: store provenance records in the exoskeleton Postgres for querying.
5. **Per-service enable/disable flags** -- Currently implicit via config presence. Future: explicit `TENTACULAR_EXOSKELETON_POSTGRES_ENABLED` style flags.
