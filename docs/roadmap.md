...
| SPIRE CA TTL tuning | **Done (2026-03-10).** Changed from `ca_ttl: 24h` (SPIRE default) to `87600h` (10 years), matching Istio's default CA lifetime. Security comes from short-lived SVIDs (4h TTL), not CA rotation frequency. Istio moved from 1-year to 10-year CA in v1.3 after operational incidents. The 10-year CA dramatically reduces trust bundle sync frequency — effectively a non-issue. Move to archive on next cleanup. | SPIRE config change | Done |
...
| Exoskeleton state reconciler | Background reconciliation loop in the MCP server (same pattern as the existing proxy reconciler) that periodically compares desired state against observed state. Handles: (1) orphaned ClusterSPIFFEIDs — compares resources with `tentacular.io/exoskeleton=true` label against active workflow Deployments, deletes orphans; (2) stale NATS authz ConfigMap entries — removes entries for tentacles that no longer exist; (3) orphaned exoskeleton Secrets. Runs on a configurable interval. This is operational housekeeping, not security posture. | None |
...
| Keycloak namespace evaluation | Re-evaluate whether Keycloak should move from `tentacular-exoskeleton` to `tentacular-system`. Keycloak shares the same ingress as the MCP server and nothing in workflow namespaces connects to it directly — only users via browser and the MCP server for OIDC discovery. Moving it to `tentacular-system` would make the OIDC validation an in-namespace call and reduce cross-namespace dependencies. Counterargument: `tentacular-system` should have minimal inbound connections. However, the ingress already routes to both MCP and Keycloak, so the attack surface argument is moot. Needs architectural review. | None |
| Security architecture document | Comprehensive cross-cutting security architecture doc covering the full threat model, trust boundaries, identity chain (deployer → workload → service), network segmentation (namespace isolation, NetworkPolicies, ingress/egress), credential lifecycle (generation, rotation, revocation), SPIFFE/SPIRE trust domain design, TLS certificate management, RBAC model, PSA enforcement, gVisor sandboxing, and audit/provenance requirements. Design goals, non-goals, and accepted risks. Should reference all four repos. Blocked on docs refactor (tentacular-docs repo with mkdocs). | Docs refactor |
...
| NATS auth callout evaluation | **Requires significant design decision.** NATS supports an Auth Callout mechanism where an external service evaluates each client connection at connect time and returns authorization decisions dynamically. This would eliminate the shared authorization ConfigMap entirely (and its concurrency issues), replacing it with on-demand evaluation. However: (1) it introduces a runtime dependency — if the auth service is down, tentacles cannot connect to NATS; (2) NATS auth callout returns a signed User JWT, which adds JWT key management back into the system when our current design intentionally avoids NATS JWTs in favor of SPIFFE mTLS; (3) the complexity tradeoff vs the current ConfigMap approach needs analysis at scale. The current ConfigMap model works and scales to hundreds of tentacles. Auth callout may be overengineering unless we hit real ConfigMap scaling limits. Needs a design doc before committing. | NATS SPIFFE mode |
