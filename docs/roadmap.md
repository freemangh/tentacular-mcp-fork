...
| SPIRE CA TTL tuning | Current `ca_ttl: 24h` (SPIRE default) causes CA rotation every ~12 hours, requiring frequent trust bundle syncs. Increase to `8760h` (1 year) for production. For context: Istio's default CA lifetime is 10 years; pre-1.3 Istio used 1 year. Security comes from short-lived SVIDs (4h), not CA rotation frequency. 1-year CA rotation is conservative and operationally sustainable. | SPIRE config change | TBD |
